

<div class="cloud" id="<?=$name;?>" data-stellar-ratio="2">
	<!-- <img src="assets/img/mycloud.svg"> -->
<!-- </div> -->

<h1><?=$name;?><?= ($name == 'Vape' ? ' in Parallax' : '');?></h1>




<div class ="opac">

	<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 455 142">
				 <title>mycloud</title>
				 <g display="inline">
				  <title>Layer 1</title>
				  <path stroke="#999999" fill="#93cdea" d="m355.205994,45.103165c-4.141205,0 -10.452209,0.179062 -18.432495,0.493973c-3.29126,-19.143188 -34.031769,-34.148003 -71.48468,-34.148003c-13.689178,0 -26.481201,2.004349 -37.365509,5.480774c-16.749084,-8.142128 -39.181152,-13.128914 -63.851471,-13.128914c-51.826813,0 -93.834106,21.97496 -93.834106,49.084915c0,2.400757 0.344711,4.759552 0.982178,7.070175c-37.450516,1.336212 -67.045937,17.641464 -67.045937,37.573807c0,20.804214 32.244484,37.67012 72.018272,37.67012c33.422623,0 242.942093,0 279.016098,0c47.558136,0 86.120636,-20.166977 86.120636,-45.050278c-0.0047,-24.874664 -38.567261,-45.04657 -86.122986,-45.04657z" id="svg_1" stroke-width="4"/>
				  <defs>
				   <radialGradient id="svg_3" spreadMethod="pad">
				    <stop stop-color="#496480" offset="0"/>
				    <stop stop-color="#b69b7f" offset="1"/>
				   </radialGradient>
				  </defs>
				 </g>
				</svg>
</div>
</div> 

<!-- <text font-weight="bold" transform="matrix(1.94713 0 0 2.21739 -231.913 -197.217)" id="svg_2" fill="#ffffff" font-size="20" font-family="arial" y="138.196078" x="234.726153" style="text-anchor: middle; opacity: 1;">
				   <?= $name; ?> </text> -->